...
The primary proof-of-concept uses reference-specific canaries:
...
For each fixed reference model, all three suspect models are evaluated on the
same canary prompt slots supplied by the reference.

This is operationally simpler than suspect-specific canaries. A reference-model
...
The canary prompt maps are reference-owned:
...
When `Qwen2.5-7B-Instruct` is the reference, all suspects are tested on prompts
`1, 2, 3`. When `Qwen2.5-14B-Instruct` is the reference, all suspects are
tested on prompts `1, 4, 5`. When `Qwen2.5-32B-Instruct` is the reference, all
suspects are tested on prompts `2, 6, 4`.

## Ordered Runs

The experiment has nine ordered runs:

| Reference model | Suspect model |
| --- | --- |
...
## 7B-Suspect Prompt Engineering Workaround

The main graph notebook follows the reference-canary protocol above: the
reference model supplies the canary prompts, and every suspect model is
evaluated on the reference model's canaries.

Under the fixed decoding setup, the `Qwen2.5-7B-Instruct` suspect curve is
undefined when the reference is `Qwen2.5-14B-Instruct` or
`Qwen2.5-32B-Instruct`, because the top-p cutoff removes token ID `16` on some
of the reference-owned canaries. To produce a visual comparison for these
cases, we use a separate fixed-budget notebook that keeps
`Qwen2.5-7B-Instruct` as the suspect but uses the 7B canaries for both curves in
each graph:

| Graph reference | Curves shown | Canary provider |
| --- | --- | --- |
| Qwen2.5-14B-Instruct | Qwen2.5-14B-Instruct reference and Qwen2.5-7B-Instruct suspect | Qwen2.5-7B-Instruct |
| Qwen2.5-32B-Instruct | Qwen2.5-32B-Instruct reference and Qwen2.5-7B-Instruct suspect | Qwen2.5-7B-Instruct |

This is a prompt-engineering workaround, not a change to the model payloads or
decoding seed. The purpose is to show the 7B suspect curve under canaries where
both target tokens remain sampleable. In this focused notebook, the
Qwen2.5-14B-Instruct reference uses `7,500` samples per context and the
Qwen2.5-32B-Instruct reference uses `15,000` samples per context.
