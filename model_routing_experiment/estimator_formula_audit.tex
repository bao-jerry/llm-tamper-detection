...
\hline
Promised model configuration & \(C_1\) & \(C_2=C_3\) & \(C_4\) \\
\hline
...
\begin{itemize}[leftmargin=*]
    ...
    \item \(z_{j,0}\) is the owned/promised-config LLM A's base next-token logit for token ID \(15\), decoded as \texttt{0}, after context \(C_j\).
    \item \(z_{j,1}\) is the owned/promised-config LLM A's base next-token logit for token ID \(16\), decoded as \texttt{1}, after context \(C_j\).
\end{itemize}
...
For a fixed promised model configuration row, the implementation needs exactly the following numerical inputs:

\begin{itemize}[leftmargin=*]
    \item Context-level values from candidate LLM B samples: \(g_1,g_2,g_4\).
    \item Promised-configuration logits from owned LLM A: \(z_{1,0}, z_{1,1}, z_{2,0}, z_{2,1}, z_{4,0}, z_{4,1}\).
\end{itemize}
