...
\begin{itemize}
\item You are the owner of LLM A, which uses a random-sampling-based next-token decoder. You suspect that black-box LLM B, which also uses a random-sampling-based next-token decoder, is a copied instance of LLM A. You do not have access to LLM B's internals (e.g. weights, decoding parameters, etc.). You may call LLM B on whatever prompts you like, however many times as you like.
...
\subsection{Formalizing the threat model assumptions}

\textbf{Formal Assumption 1:} LLM token generation is assumed to proceed by sequential random sampling from a next-token distribution.

\textbf{Formal Assumption 2:} LLM B is assumed to be a LLM whose \textit{only} implementation modifications from the base model are fixed and hidden standard token-distribution-modifying decoding parameters. In particular, this means that LLM B is not allowed to use non-parametric modifications such as secret system prompts.

\textbf{Formal Assumption 3:} "Standard token-distribution-modifying decoding parameters" is loosely defined as mainstream LLM decoding parameters that modify the token distribution. We list the formally allowed decoding parameters within the scope of this paper here:
\begin{itemize}
\item Temperature
...
\item Structured output enforcement (e.g. response\_format, json\_mode): A constrained-decoding mode that masks all tokens violating a specified output schema, forcing the output to satisfy the schema without modifying remaining logits.
\item Use-case-specific vocabulary biasing (e.g. tool/function calling toggles): Depending on the toggle setting, a fixed context-independent logit bias is applied to certain tokens. These logit biases either encourage or discourage tokens related to a specific use case.
\item top\_k
...
\begin{itemize}
\item LLM Equality: We say LLM A = LLM B, LLM B is a copy of LLM A, or LLM B is identical to LLM A if LLM A and LLM B share identical architecture, weights, vocabulary, tokenizers, and all other implementation details that constitute a \textit{base} LLM model. This does not include downstream deployment layer implementation decisions; in particular, the values of downstream deployment parameters such as temperature or logit bias are not included among the implementation details considered when establishing LLM equality.
\item Due to tokenization differences between different LLMs, when we discuss contexts, prompts, outputs, etc., by default, we are referring to their text string representations rather than any particular tokenization, unless otherwise specified.
...
The following definitions are conditional on $\displaystyle \dagger $:

\begin{itemize}
\item Suppose LLM B was repeatedly called on each context $\displaystyle C_{j}$. Let $\displaystyle N_{j}$ be the number of times LLM B was called on context $\displaystyle C_{j}$ (determined by us)
...
\item For LLM A, let $\displaystyle z_{a_{i} ,j}$ be token $\displaystyle a_{i}$'s base logit given context $\displaystyle C_{j}$ (known to us; since the user owns LLM A, we assume white-box access to LLM A, including the ability to compute its base next-token logits for chosen contexts).
\item For LLM A, Let $\displaystyle z_{b_{i} ,j}$ be token $\displaystyle b_{i}$'s base logit given context $\displaystyle C_{j}$ (known to us; since the user owns LLM A, we assume white-box access to LLM A, including the ability to compute its base next-token logits for chosen contexts).
...
\textbf{Theorem 1:} Assume $\displaystyle \dagger $. Then $\displaystyle g_{j}$ is a consistent estimator of $\displaystyle c_{j}$.
...
$\displaystyle \frac{\Delta z_{1} -\Delta z_{2}}{c_{1} -c_{2}}$
...
Let us remind ourselves what $\displaystyle \Delta b$ is: it is the difference between the sum of the logit biases of the $\displaystyle a_{i}$'s and the sum of the logit biases of the $\displaystyle b_{i}$'s. If $\displaystyle k=1$, then $\displaystyle \Delta b$ is simply the difference between the logit biases of $\displaystyle a_{1}$ and $\displaystyle b_{1}$. Let $\displaystyle a:=a_{1}$ and $\displaystyle b:=b_{1}$. In this case, $\displaystyle \Delta b$ is $\displaystyle \left( b_{a} -b_{b}\right)$. Introduce a 3rd token $\displaystyle c$. Using theorem 6, we can asymptotically compute $\displaystyle \left( b_{a} -b_{b}\right) ,\left( b_{b} -b_{c}\right) ,\left( b_{c} -b_{a}\right)$. This gives us a system of 3 linear equations where the variables are the logit biases $\displaystyle b_{a}$, $\displaystyle b_{b}$, and $\displaystyle b_{c}$. Solving gives us the logit biases of $\displaystyle a,b,c$.



Our discussion informs us that, assuming LLM A = LLM B, we can theoretically recover the individual logit biases that were applied to specific tokens in LLM B to an arbitrarily high degree of accuracy. This is perhaps a somewhat surprising identifiability result. Moreover, combining this with theorem 4 tells us that \textbf{when LLM A = LLM B, logit biases and temperature are both recoverable information, even if we did not initially know which standard token-distribution-modifying decoding parameters LLM B used.}
...
The following definitions are conditional on $\displaystyle \dagger \dagger $:
...
\begin{mdframed}
Throughout this section, we will be assuming $\dagger\dagger$ instead of $\dagger$ (which is a special case of $\dagger\dagger$; $\dagger\dagger$ is a relaxation of $\dagger$) unless otherwise stated.
\end{mdframed}
...
Note that each $\displaystyle n_{j}\left( a,i\right)$ comes from running an experiment that calls LLM B multiple times on context $\displaystyle C_{j}$ and recording the number of times that $\displaystyle a_{i,j}$ appears as an output prefix. Similar story for $\displaystyle n_{j}\left( b,i\right)$. It must be clarified that the empirical results recorded from the real-world sequence of runs done with LLM B on context $\displaystyle C_{j}$ might be data that is \textit{shared} and \textit{reused} between different $\displaystyle j$'s.



Consider the following illustrative scenario: Fix context $\displaystyle C$ and let $\displaystyle C_{j_{1}} =C_{j_{2}} =C$. For $\displaystyle C_{j_{1}}$, associate the strings $\displaystyle a_{1,j_{1}} ,...,a_{k_{j_{1}} ,j_{1}} ,b_{1,j_{1}} ,...,b_{k_{j_{1}} ,j_{1}}$. For $\displaystyle C_{j_{2}}$, associate the strings $\displaystyle a_{1,j_{2}} ,...,a_{k_{j_{2}} ,j_{2}} ,b_{1,j_{2}} ,...,b_{k_{j_{2}} ,j_{2}}$, which may or may not overlap with $\displaystyle a_{1,j_{1}} ,...,a_{k_{j_{1}} ,j_{1}} ,b_{1,j_{1}} ,...,b_{k_{j_{1}} ,j_{1}}$. Now in the real world, run LLM B repeatedly on an instance of context $\displaystyle C$. This sequence of real-world runs constitutes a single experiment. From this single experiment, we may record the number of output prefix occurrences of $\displaystyle a_{1,j_{1}} ,...,a_{k_{j_{1}} ,j_{1}} ,b_{1,j_{1}} ,...,b_{k_{j_{1}} ,j_{1}}$ (i.e. record $\displaystyle n_{j_{1}}\left( a,i\right)$ and $\displaystyle n_{j_{1}}\left( b,i\right)$) and the number of output prefix occurrences of $\displaystyle \ a_{1,j_{2}} ,...,a_{k_{j_{2}} ,j_{2}} ,b_{1,j_{2}} ,...,b_{k_{j_{2}} ,j_{2}}$ (i.e. record $\displaystyle n_{j_{2}}\left( a,i\right)$ and $\displaystyle n_{j_{2}}\left( b,i\right)$). Crucially, note that $\displaystyle g_{j_{1}}$ and $\displaystyle g_{j_{2}}$ are not built from independent, self-contained experiments. They are built from the \textit{same} experiment and therefore coupled and non-independent.



The key point is this: different $\displaystyle j$'s do not necessarily correspond to different experiments. Different $\displaystyle j$'s may represent recorded information from the \textit{same} experiment. Not merely from a\textit{ copy} of an experiment, but from the \textit{same} experiment.
...
\textbf{Definitions:}
\begin{itemize}
\item An experiment $\displaystyle E$ is a single \emph{real-world} event during which LLM B was called a finite number of times on a fixed context instance and the resulting sequence of outputs was recorded.
...
Here are some immediate facts about $\displaystyle O_{r}$:
\begin{itemize}
...
Define:
\begin{itemize}
\item For LLM A, let $\displaystyle z_{a_{i,j}}$ be token $\displaystyle a_{i,j}$'s base logit given context $\displaystyle C_{j}$ (known to us; since the user owns LLM A, we assume white-box access to LLM A, including the ability to compute its base next-token logits for chosen contexts).
\item For LLM A, Let $\displaystyle z_{b_{i,j}}$ be token $\displaystyle b_{i,j}$'s base logit given context $\displaystyle C_{j}$ (known to us; since the user owns LLM A, we assume white-box access to LLM A, including the ability to compute its base next-token logits for chosen contexts).
\item Let $\displaystyle z_{a,j} =\sum _{i=1}^{k} \ z_{a_{i,j}}$
\item Let $\displaystyle z_{b,j} =\sum _{i=1}^{k} \ z_{b_{i,j}}$
\item Let $\displaystyle \Delta z_{j} =z_{a,j} -z_{b,j}$
\end{itemize}
...
Therefore, the claim verifies that the $\displaystyle \phi $ constructed in Naive Method I satisfies step 2 of the Estimator Algorithm with known constant $\displaystyle C=0$.
...
Therefore, the claim verifies that the $\displaystyle \phi $ constructed in Naive Method II satisfies step 2 of the Estimator Algorithm with known constant $\displaystyle C=0$.
...
\textbf{Informal note:} The engineering step is finding 3 contexts and 2 tokens where the 2 tokens occur enough times for LLM B on each context to make the estimator practically viable.

\textbf{Informal note:} This is a relatively steelmanned proof of concept because the models in question belong to the same Qwen family and use the same tokenizer, preventing the experiment from exploiting quirks in tokenizer differences.

\textbf{Informal note:} Using 8-bit loading can also be framed as a conservative finite-precision stress test rather than an advantage: coarser logits make accidental coincidences and ties more likely, so any successful separation under 8-bit inference is not relying on an overly favorable continuous-logit regime.

\textbf{Informal note:} Low-precision quantization can occasionally create prompt-level probability collisions. However, estimators are multi-prompt objects and thus estimator limit collisions are far less likely. Informally speaking, this concern is further diminished if we use multiple estimators or increase the number of target tokens, as multi-estimator limit or many-token probability collisions are very unlikely.

\textbf{Informal note:} There is an important distinction between the theoretical idealization and practical implementation: the theory treats base logits as real-valued quantities in generic position, while released LLM checkpoints may expose finite-precision logits in practice. In particular, Qwen2.5 checkpoints may use native \texttt{bfloat16} weights and produce \texttt{bfloat16} next-token logits, so the empirical proof of concept should be framed as testing the method under the model's released inference precision rather than under literal continuous-real logits.

\textbf{Informal note:} We should likely replace the current "Axiom" terminology with "Theoretical Idealization Assumption" or similar language, since the generic real-valued logit condition is best understood as an idealizing assumption rather than a literal claim about released finite-precision LLM checkpoints.


\begin{center}
\fbox{\parbox{0.92\textwidth}{
\textbf{Informal draft delimiter.} We will probably split the paper here. The first paper should contain the base ownership-identification method together with a practical demo/simple experiment. The material below, beginning with the asymptotic analysis of independent $\displaystyle g_{j}$'s, likely belongs in a second paper focused on estimator design, optimization, and convergence-speed/sample-complexity analysis.
}}
\end{center}
...
\textbf{TODO:} Justify why the practically most important non-independent case is the case where the apparent non-independence comes from reusing identical $\displaystyle g_{j}$ random variables across multiple context-indexed appearances. In this case, the right practical procedure is to identify/collapse duplicate $\displaystyle g_{j}$'s and then apply the independent-case analysis to the resulting reduced experiment-indexed variables.
...
Comment: A key structural insight from this theorem is that the optimal values $\displaystyle N_{j}^{*} ,j=1:m$ are fixed proportions of $\displaystyle N$, and the proportions themselves don't depend on $\displaystyle N$. Thus, optimizing convergence speed only requires optimizing the \emph{relative proportion} of runs between experiments.



\textbf{Corollary 5}: Assume $\displaystyle N_{1} ,...,N_{m}\rightarrow \infty $. Given a run budget of $\displaystyle N_{1} +...+N_{m} =N$, we have:
\begin{enumerate}
\item $\displaystyle \frac{\partial }{\partial N_{j}} var\left( \phi \left( g_{1} ,...,g_{m}\right)\right)\rightarrow -\frac{\left(\sum _{j=1}^{m}\sqrt{q_{j}}\right)^{2}}{N^{2}}$ when $\displaystyle N_{j} =\frac{N\sqrt{q_{j}}}{\sum _{l=1}^{m}\sqrt{q_{l}}}$ for $\displaystyle j=1:m$
...
\textbf{TODO:} Heavily consider removing this subsection. The main concern is that it may create no genuinely new research insight beyond the preceding optimization discussion and may feel too engineering-oriented rather than research-oriented.
...
\textbf{TODO:} Heavily consider removing this subsection. The main concern is that it may create no genuinely new research insight beyond aggregating the preceding ideas into a pipeline and may feel too engineering-oriented rather than research-oriented.
